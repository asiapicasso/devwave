<template>
    <div class="base-input" @mouseleave="closeList">
        <input class="w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:border-blue-500" type="text"
            v-model="keyword" placeholder="J'veux cette musique..." @keydown.enter="addChosenSongOnEnter"
            @song-selected="selectSong" />
        <ul class="z-10 list-none max-h-36 overflow-y-auto p-0 m-0" v-if="songs.length > 0">
            <li v-for="song in songs.slice(0, 7)" :key="song.id" class="px-4 py-2 cursor-pointer hover:bg-gray-200"
                @click="selectSong(song)" @keydown.enter="keyEnter">
