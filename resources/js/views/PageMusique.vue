<template>
    ...
    <div>
        ...
        <div class="flex flex-col">
            <BaseVoteMusique v-for="chosenSong in chosenSongs" :key="chosenSong.id"
                :text="chosenSong.title + '&nbsp - &nbsp' + chosenSong.name" @upvote="handleUpvote(index)"
                @downvote="handleDownvote(index)" iconUp='<svg width="20" height="23" viewBox="0 0 20 23" xmlns="http://www.w3.org/2000/svg">
        ...
        </svg>' iconDown='<svg width="20" height="23" viewBox="0 0 20 23" xmlns="http://www.w3.org/2000/svg">
...
export default {
    ...
    mounted() {
        console.log('Component musiVote mounted.')
        this.fetchChosenSong();
    ...
    methods: {
        ...
        fetchChosenSong() {
            axios.get('/chosenSong')
                .then(response => {
                    this.chosenSongs = response.data;
                })
                .catch(error => {
                    console.error(error);
                });
